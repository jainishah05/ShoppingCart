...
class ProductController extends Controller
{
    ...
    public function displayProducts(Request $request,$id = null)
    {
        $perPage = 6;
        if($request->ajax())
        {
            $cat_id = $request->cat_id;
            if($request->selectedValue == 'Price:Low to High')
            {
                $products = Product::orderBy('price')->paginate($perPage);
                if (!is_null($cat_id)) { 
                    $category = Category::with('products')->where('id',$cat_id)->get();
                    $categoryProduct = $category[0]->products()->orderBy('price')->paginate($perPage);
                    return view('site.productDisplay',compact('products','categoryProduct'));
                }
                return view('site.productDisplay',compact('products'));
            }
            if($request->selectedValue == 'Price:High to Low')
            {
                $products = Product::orderBy('price','DESC')->paginate($perPage);
                if (!is_null($cat_id)) { 
                    $category = Category::with('products')->where('id',$cat_id)->get();
                    $categoryProduct = $category[0]->products()->orderBy('price','DESC')->paginate($perPage);
                    return view('site.productDisplay',compact('products','categoryProduct'));
                }
                return view('site.productDisplay',compact('products'));
            }
            else{
                $products = Product::paginate($perPage); //here,we can also use simplePaginate() method,instead of paginate method.simplePaginate() use only prev & next buttons.
                if (!is_null($cat_id)) { 
                    $category = Category::with('products')->where('id',$cat_id)->get();
                    $categoryProduct = $category[0]->products()->paginate($perPage);
                    return view('site.productDisplay',compact('products','categoryProduct'));
                }
                return view('site.productDisplay',compact('products'))->render();
            }
        }
        else{
