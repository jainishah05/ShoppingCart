...
class WishlistController extends Controller
{
    ...
    public function wishlistToCart(Request $request)
    {
        ...
        $cartItem = CartItem::where(['product_id' => $data['id'],'product_size' => $data['id'].'-'.$data['size'], 'type' => 'cart'])->count();
        if ($cartItem > 0) {
        ...
        CartItem::where(['product_id' => $data['id'],'product_size' => $size,'user_id' => Auth::user()->id, 'type' => 'wishlist'])->delete();
    ...
    {
        $product_id = Cart::instance('wishlist')->get($id)->id;
        $size = Cart::instance('wishlist')->get($id)->options->size;
        //delete item
    	Cart::instance('wishlist')->remove($id);
        //delete item in table
        CartItem::where(['product_id' => $product_id,'product_size' => $size[0].'-'.$size[1],'user_id' => Auth::user()->id, 'type' => 'wishlist'])->delete();
